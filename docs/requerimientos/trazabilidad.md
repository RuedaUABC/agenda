...
| RF-019 | Implementado | CU-006 | RN-020, RN-024, RN-026, RN-027, RN-028, RN-063 | `lib/features/horario/presentation/widgets/clase_form.dart`, `test/features/horario/clase_form_test.dart`; pendiente mejora UX para mostrar conflicto junto al campo |
...
| RF-048 | Parcial | CU-003, CU-006, CU-009, CU-010 | RN-061, RN-062, RNF-022, RNF-028, RNF-031 | `lib/features/tareas/presentation/widgets/lista_tareas_categoria.dart`, `lib/features/horario/presentation/widgets/clase_list_item.dart`, `lib/features/calendario/presentation/widgets/evento_list_item.dart`, `lib/features/configuracion/presentation/advanced_settings_widget.dart`, `test/features/configuracion/settings_effects_test.dart`, `test/features/horario/horario_widgets_test.dart`; pendiente recuperacion/deshacer y confirmacion reforzada real para borrado masivo |
| RF-049 | Parcial | CU-010 | RN-062, RNF-014, RNF-015, RNF-022, RNF-031 | `lib/features/configuracion/data/backup_file_service.dart`, `lib/features/configuracion/presentation/settings_controller.dart`, DAOs de tareas/clases/eventos, `lib/features/configuracion/presentation/advanced_settings_widget.dart`, `test/features/configuracion/settings_effects_test.dart`; implementa import/export/borrado, pendiente confirmacion reforzada verificable y tratamiento visual destructivo completo |
...
| RF-051 | Sugerido | CU-003, CU-004, CU-006, CU-009 | RN-061, RNF-028 | Auditoria Nielsen `docs/auditoria_heuristicas_nielsen.md`; evidencia actual sin `SnackBarAction` de deshacer en eliminaciones de clases/eventos y flujo de tareas |
| RF-052 | Sugerido | CU-002, CU-006, CU-009, CU-010 | RNF-011, RNF-029 | Pantallas principales usan `CircularProgressIndicator`; `EventoForm` tiene `_isSaving`; `TareaForm` y `ClaseForm` pendientes |
| RF-053 | Sugerido | CU-005, CU-007, CU-008 | RNF-012 | `lib/core/widgets/agenda_empty_state.dart`, listas de tareas, paneles de calendario/horario; pendiente accion contextual y textos especificos |
| RF-054 | Sugerido | CU-006, CU-009 | RN-063, RNF-013 | `lib/features/horario/presentation/widgets/clase_form.dart`, `lib/features/calendario/presentation/widgets/evento_form.dart`; pendiente mostrar conflicto especifico junto a fecha/hora |
| RF-055 | Sugerido | CU-010 | RN-062, RNF-026 | Ajustes y gestion de datos en `lib/features/configuracion/presentation/advanced_settings_widget.dart`; pendiente microayuda de importacion, borrado total y permisos/notificaciones |
...
| RNF-011 | Pantallas principales de features; pendiente texto contextual de carga |
| RNF-012 | Horario, calendario y listas de tareas; `test/features/diseno_material3/material3_horario_test.dart`, `test/features/diseno_material3/material3_calendario_test.dart`; pendiente acciones contextuales y textos especificos por categoria |
| RNF-013 | Formularios de tareas, clases y eventos; pendiente mover conflictos de clase a seccion Fecha y hora y detallar conflictos |
...
| RNF-022 | Preferencia global de confirmaciones aplicada a tareas/clases/eventos; pendiente confirmacion reforzada verificable en borrado total |
...
| RNF-028 | Auditoria Nielsen; pendiente `SnackBarAction` de deshacer en acciones reversibles |
| RNF-029 | `EventoForm` implementa estado de guardado; `TareaForm` y `ClaseForm` pendientes |
| RNF-030 | Sheets y grupos de botones de detalle/formularios; pendiente verificacion con anchos estrechos y texto escalado |
| RNF-031 | Dialogs y acciones destructivas en tareas/configuracion; pendiente uso consistente de `colorScheme.error` |
...
| Confirmaciones destructivas | Parcial | La preferencia global cubre eliminaciones comunes de tareas, clases y eventos; falta confirmacion reforzada verificable y recuperacion para acciones reversibles. |
...
| Ajustes avanzados | Parcial | Tema, vista inicial, densidad, inicio de semana e informacion de app estan cubiertos; confirmaciones y gestion de datos requieren refuerzo UX documentado. |
...
| Deshacer acciones reversibles | Sugerido | Agregar `SnackBarAction` para revertir eliminaciones/cambios de estado cuando sea tecnicamente seguro. |
| Confirmacion reforzada real | Parcial | Borrado total tiene dialog basico; falta accion deliberada adicional y comunicacion explicita de alcance. |
| Feedback de carga/guardado | Sugerido | Agregar textos contextuales de carga y estados `_isSaving` consistentes en tareas, clases y ajustes. |
| Diagnostico de conflictos | Sugerido | Mostrar conflictos de clase/evento junto a fecha y hora e identificar el elemento conflictivo cuando sea posible. |
| Estados vacios accionables | Sugerido | Personalizar textos por categoria y ofrecer accion de crear elemento donde aplique. |
